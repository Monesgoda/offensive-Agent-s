...
│ Target: {Target IP}:{Port} ({Service} {Version})             │
│ Technique: {CVE-ID or Technique Name}                        │
│ ATT&CK: {T-ID} ({Technique Name})                            │
│ Confidence: {Low/Medium/High/Confirmed}                      │
│ OPSEC: {QUIET/MODERATE/LOUD}                                 │
├──────────────────────────────────────────────────────────────┤
│ Step 2: Credential Access                                    │
│ Target: {Credential store / service}                         │
...
#### Validation Steps
1. Confirm {CVE-ID} on {Service} (run: {command})
2. Check if {Service} stores {credential type}
3. Verify credential privilege level against {target}
4. Test {execution method} connectivity to {target}

#### Alternative Paths at Each Step
- Step 1 alternative: {Alternative initial access vector}
- Step 3 alternative: {Alternative lateral movement method (quieter)}

#### Detection Opportunities (Blue Team)
- Step 1: WAF rule for {CVE-ID} exploit pattern
- Step 3: Monitor for {execution method} service creation (Event ID 7045)
...
[Internet] --> [DMZ: {Public-facing service}] --> [Internal: {Internal subnet}]
                                                    |
                                          [{DC IP} DC] -- [{File server IP} File Server]
                                                    |
                                          [{Workstation subnet} Workstations]
                                                    |
                                          [{Database subnet} Database Tier]

Pivot Points:
- {Host 1} ({IP}): {Zone A} to {Zone B} (confirmed)
- {Host 2} ({IP}): {Zone B} to all subnets ({trust type})
- {Jump box} ({IP}): Admin access to {sensitive tier}
```
